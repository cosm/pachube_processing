...
public class In {
	...
	public void update() { 
		remoteEnvironment = remoteXML.loadElementFrom(remoteURL);
	} 
	...
	public float getValue(int datastreamid) {
		...
		while (!foundDataElement & (k < totalEnvironmentChildren)) {
			...
			if (isDataElement(thisChild)){
				...
				if (thisChild.getAttribute("id").equals(Integer.toString(datastreamid))){
					int totalDataChildren = thisChild.countChildren();
					
					for (int i = 0; i < totalDataChildren; i++){
						if (isValueElement(thisChild.getChild(i))){
	...
	public int countDatastreams() {
		...
		for (int i = 0; i < totalEnvironmentChildren; i++){
			...
			if (isDataElement(remoteEnvironment.getChild(0).getChild(i))) total++;
		...
		while (!foundDataElement & (k < totalEnvironmentChildren)) {
			...
			if (isDataElement(thisChild)){
				...
				if (thisChild.getAttribute("id").equals(Integer.toString(id))){
					...
					for (int i = 0; i < totalDataChildren; i++){
						if (isTagElement(thisChild.getChild(i))){							
